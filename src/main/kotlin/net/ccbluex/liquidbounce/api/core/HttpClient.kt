...
object HttpClient {
    ...
    private val defaultClient = OkHttpClient.Builder()
        ...
        .build().also {
            try {
                // MCEF file utils - only available on desktop platforms
                Class.forName("net.ccbluex.liquidbounce.mcef.utils.FileUtils")
                    .getMethod("setOkHttpClient", OkHttpClient::class.java)
                    .invoke(null, it)
            } catch (e: Exception) {
                logger.debug("MCEF not available, skipping setOkHttpClient")
            }
            Authlib.client = it
        }
    ...
    suspend fun request(
        ...
        return if (progressListener == null) {
            ...
            try {
                ...
                val constructor = interceptorClass.getConstructor(progressListener.javaClass.interfaces.firstOrNull() ?: Any::class.java)
                ...
                logger.debug("OkHttpProgressInterceptor not available, falling back to standard request")
                client.newCall(request).executeAsync()
            }
        }
    }
