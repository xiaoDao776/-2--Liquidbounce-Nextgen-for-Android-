    ...
    suspend fun install(revisionId: Int, subTask: ResourceTask? = null) {
        ...
        try {
            val taskProgressUpdater = subTask?.let { subTask ->
                // Create a progress listener object - uses reflection for Android compat
                try {
                    val listenerInterface = Class.forName("net.ccbluex.liquidbounce.mcef.listeners.OkHttpProgressInterceptor\$ProgressListener")
                    java.lang.reflect.Proxy.newProxyInstance(
                        listenerInterface.classLoader,
                        arrayOf(listenerInterface)
                    ) { _, _, args ->
                        val bytesRead = args[0] as Long
                        val contentLength = args[1] as Long
