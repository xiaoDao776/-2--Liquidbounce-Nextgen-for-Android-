...
// LiquidBounce GUI Global Storage (Removed all Vape references)
object LBGuiStorage {
...
// Pure English Text Mapping (No Chinese)
object LBText {
...
// LiquidBounce Theme Color Manager
object LBColor {
    fun getAccent(): Int {
    ...
    fun border(): Int = if (LBGuiStorage.theme == 1 || LBGuiStorage.theme == 3) 0xFFDDDDDD.toInt() else 0xFF1A1A1A.toInt()
    fun hover(): Int = if (LBGuiStorage.theme == 1 || LBGuiStorage.theme == 3) 0xFFE5E5E5.toInt() else 0xFF151515.toInt()
}
...
class ClickGuiScreen : Screen(Component.literal("LiquidBounce GUI")) {
    ...
    // High-performance rounded rectangle renderer
    ...
    // Recursively read module config values
    ...
    private fun isColorValue(v: Value<*>): Boolean {
        val obj = getActualValue(v) ?: return false
        return obj is Color || obj.javaClass.simpleName.contains("Color", true) || v.name.contains("Color", true)
    }
    private fun extractColor(v: Value<*>): Color {
        val obj = getActualValue(v) ?: return Color.WHITE
        if (obj is Color) return obj
        if (obj is Number) return Color(obj.toInt(), true)
        try {
            val m = obj.javaClass.methods.find { it.name == "getRGB" || it.name == "rgb" }
            if (m != null) return Color((m.invoke(obj) as Number).toInt(), true)
        } catch (_: Exception) {}
        return Color.WHITE
    }
    private fun updateColorValue(v: Value<*>, color: Color) {
        try {
            ...
            if (actual is Color) @Suppress("UNCHECKED_CAST") (v as Value<Color>).set(color)
            else @Suppress("UNCHECKED_CAST") (v as Value<Int>).set(color.rgb)
        } catch (_: Exception) {}
    }
    ...
    // Only Floating Window UI (Removed old centered panel)
    ...
    override fun mouseDragged(pMouseX: Double, pMouseY: Double, button: Int) {
        super.mouseDragged(pMouseX, pMouseY, button)
        val dragId = LBGuiStorage.dragWin ?: return
        val nx = (pMouseX - LBGuiStorage.dragOffX).toInt().coerceAtLeast(0)
        val ny = (pMouseY - LBGuiStorage.dragOffY).toInt().coerceAtLeast(0)
