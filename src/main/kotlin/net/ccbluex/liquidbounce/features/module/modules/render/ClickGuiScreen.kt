    ...
    private fun getGroupChildren(v: Value<*>): List<Value<*>> {
        ...
        try {
            for (m in v.javaClass.methods) {
                if (m.parameterCount == 0 && Collection::class.java.isAssignableFrom(m.returnType)) {
                    val res = m.invoke(v) as? Collection<*>
                    if (res != null) list.addAll(res.filterIsInstance<Value<*>>())
                } else if (m.parameterCount == 0 && m.returnType.isArray) {
                    val res = m.invoke(v) as? Array<*>
                    if (res != null) list.addAll(res.filterIsInstance<Value<*>>())
                }
            }
