...
object FontManager {
     ...
     * On Android, we use Droid Sans (Android's default system font) or Roboto.
     */
    private val COMMON_FONT by AsyncLazy {
        runCatching {
            when {
                PlatformUtils.IS_ANDROID || PlatformUtils.IS_POJAV -> {
                    // Android system fonts
                    runCatching { systemFont("Droid Sans") }
                        .recover { systemFont("Roboto") }
                        .recover { systemFont("sans-serif") }
                        .getOrElse { systemFont("Arial") }
                }
     ...
     * On Android, we use Droid Sans Fallback or Noto Sans CJK.
     */
    private val CJK_FONT by AsyncLazy {
        runCatching {
            when {
                PlatformUtils.IS_ANDROID || PlatformUtils.IS_POJAV -> {
                    // Android CJK fonts
                    runCatching { systemFont("Droid Sans Fallback") }
                        .recover { systemFont("Noto Sans CJK SC") }
                        .recover { systemFont("Noto Sans CJK") }
                        .getOrNull()
                }
