...
/**
 * Check if TinyFileDialogs is available (it requires native libraries
 * that are not available on Android/PojavLauncher).
 */
private val tinyfdAvailable: Boolean by lazy {
    PlatformUtils.SUPPORTS_NATIVE_DIALOGS &&
    try {
...
class ErrorHandler private constructor(
    ...
    fun buildAndShowMessage(): Boolean {
        ...
        return when {
            ...
            // On Android / PojavLauncher, TinyFileDialogs is not available.
            // Just log the error and exit gracefully.
            !tinyfdAvailable -> {
                logger.error(message)
                if (needToReport) {
                    logger.warn("Please report this issue manually.")
                }
                false
            }
