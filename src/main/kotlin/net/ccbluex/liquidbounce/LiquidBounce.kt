...
import net.ccbluex.liquidbounce.utils.client.android.AndroidCompat
...
object LiquidBounce : EventListener {
    ...
    private suspend fun prepareGuiStage(
        ...
        // On Android, skip JCEF browser backend initialization
        if (AndroidCompat.isFeatureAvailable("jcef_browser")) {
            BrowserBackendManager.init()
        } else {
            logger.info("Skipping JCEF browser backend initialization (not available on Android)")
        }
        ...
        taskManager = TaskManager(ioScope).apply {
            ...
            // Skip deep learning on Android - DJL/PyTorch has no Android support
            if (AndroidCompat.isFeatureAvailable("deep_learning")) {
                ...
                logger.info("Skipping deep learning initialization (not available on Android)")
            }
    ...
    private val startHandler = handler<ClientStartEvent> {
        runCatching {
            ...
            // Print Android-specific information if running on Android
            AndroidCompat.printAndroidInfo()
